...
# Determinism seedcheck (only 2a/trans).
bash "${RUN_TMUX}" "${HOST}" "unify_seedcheck__${variant_ifsa}__bci_iv_2a__trans" \
  "${ENV_PREFIX} python -m eapp.run experiment=${variant_ifsa} dataset=bci_iv_2a protocol.target_data_usage=transductive_unlabeled_all method.trigger_tau=0.0 runtime.n_jobs=1 runtime.seed=43 runtime.results_dir=results_seedcheck_final_v2"
