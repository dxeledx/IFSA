...
variants=(
  ...
  ifsa_v13_tg_beta0p5_full
  ifsa_v13_tg_beta1_partial
  ifsa_v13_tg_beta1_full_arith
)

for v in "${variants[@]}"; do
  "${RUN_TMUX}" "${HOST}" "ifsa_roundC__${v}__2a__trans" \
  ...
  "${RUN_TMUX}" "${HOST}" "ifsa_roundC__${v}__2b__online" \
