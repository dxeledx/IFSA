...
variants=(
  ifsa_v24_track_scale_tau0p4_hold0p5_disc0p001
  ifsa_v24_track_scale_tau0p6_hold0p5_disc0p001
)

for v in "${variants[@]}"; do
  "${RUN_TMUX}" "${HOST}" "ifsa_roundM__${v}__2a__trans" \
  ...
  "${RUN_TMUX}" "${HOST}" "ifsa_roundM__${v}__2b__online" \
