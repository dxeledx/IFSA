...
variants=(
  ifsa_v19_ctrl
  ifsa_v19_split_hold_q0p95
  ifsa_v19_split_hold_q0p90
  ifsa_v19_split_scale_q0p95
)

for v in "${variants[@]}"; do
  "${RUN_TMUX}" "${HOST}" "ifsa_roundH__${v}__2a__trans" \
  ...
  "${RUN_TMUX}" "${HOST}" "ifsa_roundH__${v}__2b__online" \
