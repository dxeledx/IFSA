...
from eapp.utils.spd import invsqrtm_spd, log_eigvals_spd, log_euclidean_mean
...
class RASignalAligner:
    """Riemannian re-centering at identity using log-Euclidean mean."""

    def __init__(self, cov_cfg: CovarianceConfig):
        self.cov_cfg = cov_cfg
        self.matrix: np.ndarray | None = None
        self.metrics: RAMetrics | None = None

    def fit(self, x: np.ndarray) -> RASignalAligner:
        covs = compute_covariances(x, self.cov_cfg)
        mean_cov = log_euclidean_mean(covs, eps=self.cov_cfg.epsilon)
    ...
    def transform(self, x: np.ndarray) -> np.ndarray:
        if self.matrix is None:
            raise RuntimeError("RASignalAligner not fit")
