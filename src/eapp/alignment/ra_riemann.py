...
class RARiemannMetrics:
    spec_var_before: float
    spec_var_after: float
    control_energy: float


class RARiemannSignalAligner:
    """Riemannian re-centering at identity using affine-invariant Riemannian mean.

    This is closer to the TLCenter operator described in:
    "Transfer Learning: A Riemannian Geometry Framework With Applications to
    Brain–Computer Interfaces" (Zanini et al.).
    """

    def __init__(self, cov_cfg: CovarianceConfig):
        self.cov_cfg = cov_cfg
        self.matrix: np.ndarray | None = None
        self.metrics: RARiemannMetrics | None = None

    def fit(self, x: np.ndarray) -> RARiemannSignalAligner:
        try:
            from pyriemann.utils.mean import mean_riemann
        except Exception as exc:  # pragma: no cover
            raise RuntimeError(
                "RARiemannSignalAligner requires `pyriemann` (failed to import mean_riemann)"
            ) from exc
    ...
    def transform(self, x: np.ndarray) -> np.ndarray:
        if self.matrix is None:
            raise RuntimeError("RARiemannSignalAligner not fit")
